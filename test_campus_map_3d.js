...
await sleep(500);
const nightLights=await ev(`(()=>{const d=window.__SC_CAMPUS_MAP_3D__.frame.contentWindow.__DAVIS_TWIN_DEBUG__;const roots=d.traffic&&d.traffic.set&&d.traffic.set.roots||[];const first=roots.find(r=>r&&r.userData&&r.userData.vehicleLights);const h=first&&first.userData.vehicleLights.head||[],t=first&&first.userData.vehicleLights.tail||[],tv=d.transit&&d.transit.vehicles||[];const npcHead=h.some(m=>m.emissiveIntensity>.2),npcTail=t.some(m=>m.emissiveIntensity>.2),npcGlow=!!(first&&first.userData.vehicleLightRig&&first.userData.vehicleLightRig.headGlow.some(s=>s.material.opacity>.1)&&first.userData.vehicleLightRig.tailGlow.some(s=>s.material.opacity>.1));const transitOn=tv.length===2&&tv.every(v=>(v.lightMaterials||[]).some(m=>m.emissiveIntensity>.2)&&v.lightRig.headEmit.some(l=>l.intensity>.2)&&v.lightRig.tailEmit.some(l=>l.intensity>.1));return {night:d.ST.night,npcOn:npcHead&&npcTail&&npcGlow,transitOn:transitOn};})()`);
