...
function notifyUiOfSync(type = 'remote') {
  try { window.dispatchEvent(new CustomEvent('schoolcenter:data-sync-changed', { detail: { type } })); } catch (_) {}
}
...
async function syncAfterLocalChange() {
  try { await fullTwoWaySync(); } catch (_) { /* retry on next mutation / timer / wake */ }
}
...
export async function startAutomaticDataSync() {
  ...
  try { await fullTwoWaySync(); } catch (_) {}
  if (generation !== syncGeneration || !autoSyncStarted) return false;

  const wake = () => { if (autoSyncStarted) fullTwoWaySync().catch(() => {}); };
  ...
  periodicSyncTimer = setInterval(() => {
    if (autoSyncStarted) fullTwoWaySync().catch(() => {});
  }, PERIODIC_SYNC_MS);
