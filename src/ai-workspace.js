...
export class GeminiLiveTranscriber {
  constructor({ apiKey, onStatus, onInterim, onFinal, onError } = {}) {
    this.apiKey = apiKey;
  ...
  async start() {
    ...
    if (!this.apiKey) throw new Error('Add your Gemini API key in Settings.');
    ...
    await new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('Timed out connecting to live transcription.')), 10000);
      this.ws.onopen = () => {
        clearTimeout(timer);
    ...
    this.ws.onerror = (e) => this.onError(e?.message || 'Live transcription connection error.');
