  ...
  return `
    <header class="app-header">
      ...
      <div class="header-actions">
        <button class="ai-launch-btn ai-launch-combined" id="header-ai-btn" type="button" title="Open AI workspace" aria-label="Open AI workspace">
          ${icon('spark')}<span class="ai-launch-dot ${aiState}" aria-hidden="true"></span>
        </button>
        <button class="icon-btn sm" id="quick-search-btn" type="button" title="Search" aria-label="Open search">${icon('search')}</button>
      </div>
    </header>`;
}
...
function renderBottomNav() {
  ...
  return `<nav class="bottom-nav-wrap" aria-label="Primary navigation"><div class="bottom-nav">${tabs.map(t => `<button class="nav-item ${state.view===t.id?'active':''}" data-nav="${t.id}" type="button">${icon(t.icon)}<span>${t.label}</span></button>`).join('')}</div></nav>`;
}
  ...
  return `
    <div class="settings-page">
      ...
      <div class="panel settings-card">
        <h3 class="headfont">Search</h3>
        <div class="search-input-wrap"><span class="search-icon-left">${icon('search')}</span><input type="text" class="search-input" id="universal-search-input" placeholder="Search courses, assignments, notes..." value="${state.searchQuery || ''}"></div>
        ${state.searchQuery ? renderSettingsSearchResults() : ''}
      </div>
      <div class="panel settings-card">
      ...
      <div class="panel settings-card">
        ...
        <div class="theme-row" id="theme-row">${Object.entries(THEME_PRESETS).map(([k,t])=>`<button class="swatch ${currentTheme.label===t.label?'active':''}" data-theme-key="${k}" type="button" style="background:linear-gradient(135deg, ${t.accent}, ${t.lava[1]})" title="${t.label}" aria-label="${t.label}"></button>`).join('')}</div>
...
function attachEventHandlers() {
  ...
  if (headerAiBtn) headerAiBtn.addEventListener('click', () => { state.aiAssistantOpen = true; state.aiConnectionState = 'idle'; render(); setTimeout(() => document.getElementById('ai-chat-input')?.focus(), 50); });
  const closeAiBtn = document.getElementById('close-ai-assistant-modal');
  if (closeAiBtn) closeAiBtn.addEventListener('click', async () => { await stopAiLiveTranscription(); state.aiAssistantOpen = false; render(); });
  const aiOverlay = document.getElementById('ai-assistant-modal-overlay');
  if (aiOverlay) aiOverlay.addEventListener('click', async e => { if (e.target === aiOverlay) { await stopAiLiveTranscription(); state.aiAssistantOpen = false; render(); } });
  ...
  if (assignmentForm) assignmentForm.addEventListener('submit', e => {
    e.preventDefault(); const fd = new FormData(assignmentForm); assignmentsManager.createAssignment({ title: fd.get('title'), courseId: fd.get('courseId'), dueDate: fd.get('dueDate'), priority: fd.get('priority') || 'medium', description: fd.get('description') || '' }); state.assignmentModalOpen = false; showToast('Assignment saved ✓'); render();
  });
  ...
  if (noteForm) noteForm.addEventListener('submit', e => {
    e.preventDefault(); const fd = new FormData(noteForm); notesManager.createNote({ title: fd.get('title'), content: fd.get('content'), courseId: fd.get('courseId') || null }); state.noteModalOpen = false; showToast('Note saved ✓'); render();
  });
  const closeAssignmentDetail = document.getElementById('close-assignment-detail-modal');
  if (closeAssignmentDetail) closeAssignmentDetail.addEventListener('click', () => { state.assignmentDetailId = null; render(); });
  const assignmentDetailOverlay = document.getElementById('assignment-detail-modal-overlay');
