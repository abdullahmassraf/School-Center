...
import { VoiceMemoStore, buildWaveformPeaks, formatMemoTime, revokeObjectUrl } from './voice-memos.js';
...
let voiceMemoStore = new VoiceMemoStore();
const voicePlayerUrls = new Map();
const voicePlayerElements = new Map();
const voiceWaveAnimation = new Map();
...
function render() {
  ...
  try {
    ...
    try {
      ...
      } else if (state.view === 'voice-memos') {
        viewHtml = renderVoiceMemosView();
...
function renderCaptureFAB() {
  return `
    <button class="fab-capture" id="fab-capture-btn" type="button" aria-label="Open Quick Capture" title="Quick Capture">
      ${icon('plus')}
    </button>
  `;
}
...
/* =========================================================================
   VIEW 4: STANDALONE VOICE MEMOS
   ========================================================================= */
function renderVoiceMemosView() {
  const memos = Array.isArray(state.voiceMemos) ? state.voiceMemos : [];
  const pending = state.pendingVoiceRecording;
  const courseOptions = [{ id: '', code: 'No course', name: '' }, ...COURSES].map(c =>
    `<option value="${c.id}" ${c.id === (state.voiceMemoCourseId || '') ? 'selected' : ''}>${c.code}${c.name ? ` — ${c.name}` : ''}</option>`
  ).join('');

  return `
    <div class="voice-memos-page">
      <div class="panel voice-memos-hero">
        <div class="voice-memos-hero-copy">
          <div class="eyebrow">Standalone audio workspace</div>
          <h2 class="headfont">Voice Memos</h2>
          <p>Record lectures, quick ideas, and study notes independently from any course view. Choose a course tag only when you want one.</p>
        </div>
        <div class="voice-memo-storage-pill"><span class="jobs-pill-dot"></span> Saved on this device</div>
      </div>

      <div class="panel voice-recorder-panel">
        <div class="voice-recorder-header">
          <div>
            <div class="field-label">Memo title</div>
            <input id="voice-memo-title" class="modal-input" type="text" maxlength="120" placeholder="e.g. Lecture 4 — RREF walkthrough" value="${state.voiceMemoTitle || ''}">
          </div>
          <div>
            <div class="field-label">Course tag</div>
            <select id="voice-memo-course" class="modal-input">${courseOptions}</select>
          </div>
        </div>

        <div class="voice-recorder-stage ${recorderInstance?.state === 'recording' ? 'is-recording' : ''}">
          <div class="voice-live-state"><span class="voice-live-dot"></span><span id="voice-live-status">${recorderInstance?.state === 'recording' ? 'Recording' : recorderInstance?.state === 'paused' ? 'Paused' : 'Ready to record'}</span></div>
          <div class="voice-timer-large" id="audio-timer-display">${formatMemoTime(recorderInstance?.elapsedSeconds || 0)}</div>
          <canvas id="audio-canvas" class="voice-waveform-canvas" aria-label="Live recording audio visualizer"></canvas>
          <div class="voice-controls-row">
            <button class="voice-ctrl-btn pause-btn" id="voice-pause-btn" type="button" aria-label="Pause or resume recording" title="Pause / Resume" ${recorderInstance?.state === 'recording' || recorderInstance?.state === 'paused' ? '' : 'disabled'}>${icon(recorderInstance?.state === 'paused' ? 'play' : 'pause')}</button>
            <button class="voice-ctrl-btn record-main ${recorderInstance?.state === 'recording' ? 'recording' : ''}" id="record-btn" type="button" aria-label="${recorderInstance?.state === 'recording' ? 'Pause recording' : 'Start recording'}" title="${recorderInstance?.state === 'recording' ? 'Pause recording' : 'Start recording'}">${icon(recorderInstance?.state === 'recording' ? 'pause' : 'mic')}</button>
            <button class="voice-ctrl-btn pause-btn" id="voice-stop-btn" type="button" aria-label="Stop recording" title="Stop recording" ${recorderInstance?.state === 'recording' || recorderInstance?.state === 'paused' ? '' : 'disabled'}>${icon('check')}</button>
          </div>
          <div class="voice-recorder-help" id="record-status-label">${recorderInstance?.state === 'recording' ? 'Microphone input is live.' : recorderInstance?.state === 'paused' ? 'Recording is paused.' : 'Your recording will remain separate from Linear Algebra Studio.'}</div>
        </div>

        ${pending ? `
          <div class="voice-pending-card" id="voice-pending-card">
            <div class="voice-pending-head"><div><b>Recording captured</b><div class="voice-meta">${formatMemoTime(pending.duration)}${pending.transcript?.length ? ` · ${pending.transcript.length} transcript chunks` : ''}</div></div><span class="badge">Ready to save</span></div>
            <audio id="voice-pending-audio" class="voice-pending-audio" controls preload="metadata" src="${pending.url}"></audio>
            <div class="voice-pending-actions">
              <button class="btn-ghost" id="discard-voice-recording" type="button">Discard</button>
              <button class="btn-primary" id="save-voice-recording" type="button">Save Voice Memo</button>
            </div>
          </div>
        ` : ''}
      </div>
      ...
      <div class="panel">
        <div class="section-head-row">
          <div><h2 class="headfont">Saved Memos</h2><div class="section-sub">${memos.length} saved on this browser</div></div>
          <button class="btn-ghost" id="refresh-voice-memos" type="button">Refresh</button>
        </div>
        <div class="voice-memo-list">
          ${memos.length ? memos.map(renderVoiceMemoCard).join('') : `
            <div class="empty-state-card"><div class="empty-state-icon">${icon('mic')}</div><b>No voice memos yet</b><span>Record your first memo above and it will appear here.</span></div>
          `}
        </div>
      </div>
    </div>
...
function renderVoiceMemoCard(memo) {
  const peaks = Array.isArray(memo.peaks) ? memo.peaks : [];
  const course = memo.courseId ? courseById(memo.courseId) : null;
  const activePlayer = voicePlayerElements.get(memo.id);
  const speed = activePlayer?.playbackRate || 1;
  return `
    <article class="voice-memo-card" data-memo-id="${memo.id}">
      <div class="voice-memo-card-head">
        <div class="voice-memo-title-wrap">
          <input class="voice-memo-title-input" data-memo-title="${memo.id}" value="${String(memo.title || 'Voice Memo').replace(/"/g, '&quot;')}" aria-label="Voice memo title">
          <div class="voice-memo-meta"><span>${new Date(memo.savedAt || memo.createdAt || Date.now()).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</span><span>·</span><span>${formatMemoTime(memo.duration)}</span>${course ? `<span>·</span><span style="color:${course.accent}">${course.code}</span>` : ''}</div>
        </div>
        <button class="icon-btn sm delete-memo-btn" data-memo-delete="${memo.id}" type="button" aria-label="Delete ${String(memo.title || 'voice memo').replace(/"/g, '&quot;')}" title="Delete memo">${icon('trash')}</button>
      </div>
      <div class="voice-memo-controls-row">
        <button class="voice-mini-btn memo-play-btn" data-memo-play="${memo.id}" type="button" aria-label="Play or pause ${String(memo.title || 'voice memo').replace(/"/g, '&quot;')}">${icon(activePlayer?.paused === false ? 'pause' : 'play')}</button>
        <button class="voice-mini-btn" data-memo-rewind="${memo.id}" type="button" aria-label="Rewind 10 seconds">${icon('rewind')} <span>10</span></button>
        <button class="voice-mini-btn" data-memo-forward="${memo.id}" type="button" aria-label="Forward 10 seconds">${icon('forward')} <span>10</span></button>
        <div class="voice-speed-toggle" role="group" aria-label="Playback speed">
          ${[1, 1.25, 1.5, 2].map(v => `<button class="voice-speed-btn ${speed === v ? 'active' : ''}" data-memo-speed="${memo.id}" data-speed="${v}" type="button">${v}x</button>`).join('')}
        </div>
      </div>
      <canvas class="memo-waveform-canvas" data-memo-wave="${memo.id}" data-peaks='${JSON.stringify(peaks)}' width="840" height="96" aria-label="Interactive waveform for ${String(memo.title || 'voice memo').replace(/"/g, '&quot;')}"></canvas>
      <div class="voice-memo-time-row"><span data-memo-current="${memo.id}">00:00</span><span data-memo-duration="${memo.id}">${formatMemoTime(memo.duration)}</span></div>
      <select class="voice-memo-course-select" data-memo-course="${memo.id}" aria-label="Associated course">
        <option value="">No course tag</option>
        ${COURSES.map(c => `<option value="${c.id}" ${c.id === (memo.courseId || '') ? 'selected' : ''}>${c.code} — ${c.name}</option>`).join('')}
      </select>
    </article>
  `;
}
...
function renderMoreView() {
  ...
  return `
    ...
    <!-- Practice & Focus shortcuts -->
    <div class="panel">
      <h2 class="headfont">Study Tools</h2>
      <div class="tool-launch-grid">
        <button class="tool-launch-card" id="more-practice-btn" type="button">
          ${icon('spark')}
          <span><b>Practice Studio</b><small>Open Linear Algebra Studio</small></span>
          ${icon('chevronRight')}
        </button>
        <button class="tool-launch-card" id="more-voice-btn" type="button">
          ${icon('mic')}
          <span><b>Voice Memos</b><small>Record and manage audio</small></span>
          ${icon('chevronRight')}
        </button>
      </div>
    </div>
...
export function buildGeminiSystemPrompt(mode) {
  const scheduleSummary = FALL_2026_SCHEDULE.map(s => 
    `• ${s.dayName}: ${s.courseCode} (${s.type}) ${s.start}–${s.end} in Room ${s.room} (Instructor: ${s.instructor})`
  ).join('\n');

  const courseSummary = COURSES.map(c => 
    `• ${c.code}: ${c.name} (Instructor: ${c.instructor || 'TBA'}, ID: ${c.id})`
  ).join('\n');

  const pendingAssignments = (typeof assignmentsManager !== 'undefined' ? assignmentsManager.getAll() : [])
    .filter(a => a.status !== 'completed')
    .map(a => `• ${a.title} (Course: ${a.courseId}, Due: ${a.dueDate}, Priority: ${a.priority || 'Normal'})`)
    .join('\n');
    ...
    <div class="modal-overlay open" id="ai-assistant-modal-overlay">
      <div class="modal-box ai-modal-box">
        <div class="ai-head-tabs">
          ...
          <button class="ai-tab-btn ${isDev ? 'active' : ''}" id="ai-tab-developer">🛠 Developer Mode</button>
        ...
        <div class="ai-chat-stream" id="ai-chat-stream">
          ${!isDev ? `
            <div class="ai-bubble assistant">
              ...
              <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:10px;">
                <button class="btn-ghost ai-prompt-chip" data-prompt="What classes do I have today and where are they located?" style="font-size:0.75rem;padding:4px 10px;min-height:28px;">📅 Today's Classes</button>
                <button class="btn-ghost ai-prompt-chip" data-prompt="Explain Elementary Row Operations and Reduced Row Echelon Form for Linear Algebra" style="font-size:0.75rem;padding:4px 10px;min-height:28px;">📐 Linear Algebra (RREF)</button>
                <button class="btn-ghost ai-prompt-chip" data-prompt="What are my upcoming assignment deadlines and priorities?" style="font-size:0.75rem;padding:4px 10px;min-height:28px;">📋 Priority Tasks</button>
                <button class="btn-ghost ai-prompt-chip" data-prompt="Explain the First and Second Laws of Thermodynamics for Energy Systems" style="font-size:0.75rem;padding:4px 10px;min-height:28px;">⚡ Thermodynamics Laws</button>
              </div>
            </div>
          ` : `
            <div class="ai-bubble assistant">
              ...
              <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:12px;">
                <button class="btn-ghost ai-dev-action-chip" data-dev-cmd="Run a full audit of my Fall 2026 schedule and confirm room locations" style="font-size:0.75rem;padding:4px 10px;min-height:28px;">🧪 Schedule Audit</button>
                <button class="btn-ghost ai-dev-action-chip" data-dev-cmd="Verify my 5 enrolled courses and check for any duplicates" style="font-size:0.75rem;padding:4px 10px;min-height:28px;">📋 Verify 5 Courses</button>
                <button class="btn-ghost ai-dev-action-chip" data-dev-cmd="Inspect the translucent glass tokens and optical blur parameters" style="font-size:0.75rem;padding:4px 10px;min-height:28px;">🎨 Glass Inspection</button>
              </div>
            </div>
          `}
...
function attachEventHandlers() {
  ...
  if (capAudio) capAudio.addEventListener('click', () => {
    state.quickCaptureOpen = false;
    state.view = 'voice-memos';
    state.courseId = null;
    render();
  });

  const morePracticeBtn = document.getElementById('more-practice-btn');
  if (morePracticeBtn) morePracticeBtn.addEventListener('click', () => { state.view = 'practice'; render(); });
  const moreVoiceBtn = document.getElementById('more-voice-btn');
  if (moreVoiceBtn) moreVoiceBtn.addEventListener('click', () => { state.view = 'voice-memos'; state.courseId = null; render(); });

  // Standalone Voice Memos
  attachVoiceMemoHandlers();
  ...
  if (!window._sc_keydown_attached) {
    ...
    window.addEventListener('keydown', (e) => {
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
        ...
        if (state.spotlightSearchOpen || state.monthCalendarOpen || state.syncDrawerOpen || state.aiAssistantOpen || state.quickCaptureOpen || state.assignmentModalOpen || state.noteModalOpen || state.flashcardsModalOpen || state.assignmentDetailId) {
          state.spotlightSearchOpen = false;
          state.monthCalendarOpen = false;
          state.syncDrawerOpen = false;
          state.aiAssistantOpen = false;
...
async function initializeVoiceMemos() {
  try {
    await voiceMemoStore.initialize();
    state.voiceMemos = await voiceMemoStore.list();
    render();
  } catch (e) {
    console.warn('Voice memo storage unavailable:', e);
  }
}

function refreshVoiceMemosInMemory() {
  return voiceMemoStore.list().then(memos => {
    state.voiceMemos = memos;
    render();
  }).catch(e => console.warn('Unable to refresh voice memos:', e));
}

function drawMemoWave(canvas, memo, audio) {
  if (!canvas) return;
  ...
  const height = Math.max(1, Math.floor((rect.height || 96) * dpr));
  canvas.width = width;
  canvas.height = height;
  ctx.clearRect(0, 0, width, height);

  const peaks = Array.isArray(memo.peaks) && memo.peaks.length ? memo.peaks : new Array(120).fill(0.12);
  const mid = height / 2;
  const barWidth = Math.max(2, width / peaks.length - 2);
  const played = audio?.duration ? Math.max(0, Math.min(1, audio.currentTime / audio.duration)) : 0;
  const left = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#8B7CF6';
  const right = getComputedStyle(document.documentElement).getPropertyValue('--accent-2').trim() || '#34D1BF';

  for (let i = 0; i < peaks.length; i++) {
    const x = i * (barWidth + 2);
    const magnitude = Math.max(0.08, Number(peaks[i]) || 0.08);
    const h = magnitude * height * 0.78;
    const active = i / peaks.length <= played;
    ctx.fillStyle = active ? left : 'rgba(255,255,255,0.20)';
    ctx.globalAlpha = active ? 0.92 : 0.55;
    ctx.beginPath();
    const y = mid - h / 2;
    if (ctx.roundRect) ctx.roundRect(x, y, barWidth, h, Math.min(4, barWidth / 2));
  ...
  ctx.beginPath();
  ctx.moveTo(0, mid); ctx.lineTo(width, mid); ctx.stroke();
}

function renderMemoWaveLoop(memoId) {
  const memo = state.voiceMemos.find(m => m.id === memoId);
  const audio = voicePlayerElements.get(memoId);
  const canvas = document.querySelector(`[data-memo-wave="${memoId}"]`);
  if (!memo || !canvas) return;
  drawMemoWave(canvas, memo, audio);
  if (audio && !audio.paused) {
    const raf = requestAnimationFrame(() => renderMemoWaveLoop(memoId));
    voiceWaveAnimation.set(memoId, raf);
  } else {
    const raf = voiceWaveAnimation.get(memoId);
    if (raf) cancelAnimationFrame(raf);
    voiceWaveAnimation.delete(memoId);
  }
}

async function attachVoiceMemoHandlers() {
  const titleInput = document.getElementById('voice-memo-title');
  if (titleInput) titleInput.addEventListener('input', e => { state.voiceMemoTitle = e.target.value; });
  const courseInput = document.getElementById('voice-memo-course');
  if (courseInput) courseInput.addEventListener('change', e => { state.voiceMemoCourseId = e.target.value; });
  ...
  const statusEl = document.getElementById('record-status-label');

  if (recordBtn) recordBtn.addEventListener('click', async () => {
    if (!recorderInstance || recorderInstance.state === 'stopped') {
      recorderInstance = new VoiceRecorder({
        onTimeUpdate: (sec) => {
          const timer = document.getElementById('audio-timer-display');
          if (timer) timer.textContent = formatMemoTime(sec);
        }
      });
    }
    if (recorderInstance.state === 'idle') {
      try {
        await recorderInstance.startRecording(canvas);
        render();
        setTimeout(() => recorderInstance?.startCanvasVisualizer(document.getElementById('audio-canvas')), 0);
      } catch (e) {
        showToast(e.message || 'Microphone access failed.');
      }
    } else if (recorderInstance.state === 'recording') {
      recorderInstance.pauseRecording();
      render();
    } else if (recorderInstance.state === 'paused') {
      recorderInstance.resumeRecording(document.getElementById('audio-canvas'));
      render();
    }
  });
  ...
  if (pauseBtn) pauseBtn.addEventListener('click', () => {
    ...
    if (recorderInstance.state === 'recording') recorderInstance.pauseRecording();
    else if (recorderInstance.state === 'paused') recorderInstance.resumeRecording(document.getElementById('audio-canvas'));
    render();
  });

  if (stopBtn) stopBtn.addEventListener('click', async () => {
    if (!recorderInstance || !['recording', 'paused'].includes(recorderInstance.state)) return;
    const result = await recorderInstance.stopRecording();
    if (!result) return;
    state.pendingVoiceRecording = result;
    render();
  });

  const discard = document.getElementById('discard-voice-recording');
  if (discard) discard.addEventListener('click', () => {
    if (state.pendingVoiceRecording?.url) revokeObjectUrl(state.pendingVoiceRecording.url);
    state.pendingVoiceRecording = null;
    recorderInstance = null;
    render();
  });

  const save = document.getElementById('save-voice-recording');
  if (save) save.addEventListener('click', async () => {
    const pending = state.pendingVoiceRecording;
    if (!pending?.blob) return;
    const title = (state.voiceMemoTitle || '').trim() || `Voice Memo ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
    save.disabled = true;
    save.textContent = 'Saving...';
    try {
      const peaks = await buildWaveformPeaks(pending.blob, 140);
      const memo = {
        ...
        courseId: state.voiceMemoCourseId || null,
        mimeType: pending.blob.type || 'audio/webm',
        blob: pending.blob,
        peaks,
        transcript: pending.transcript || []
      };
      await voiceMemoStore.save(memo);
      revokeObjectUrl(pending.url);
      state.pendingVoiceRecording = null;
      state.voiceMemoTitle = '';
      recorderInstance = null;
      await refreshVoiceMemosInMemory();
      showToast('Voice memo saved ✓');
    } catch (e) {
      console.error(e);
      save.disabled = false;
      save.textContent = 'Save Voice Memo';
      showToast('Could not save the voice memo on this device.');
    }
  });

  const refresh = document.getElementById('refresh-voice-memos');
  if (refresh) refresh.addEventListener('click', refreshVoiceMemosInMemory);

  document.querySelectorAll('[data-memo-delete]').forEach(btn => btn.addEventListener('click', async () => {
    const id = btn.getAttribute('data-memo-delete');
    const memo = state.voiceMemos.find(m => m.id === id);
    if (!memo) return;
    const ok = window.confirm(`Delete “${memo.title || 'Voice Memo'}”? This cannot be undone.`);
    if (!ok) return;
    try {
      const audio = voicePlayerElements.get(id);
      if (audio) audio.pause();
      revokeObjectUrl(voicePlayerUrls.get(id));
      voicePlayerUrls.delete(id);
      voicePlayerElements.delete(id);
      await voiceMemoStore.remove(id);
      await refreshVoiceMemosInMemory();
      showToast('Voice memo deleted');
    } catch (e) {
      showToast('Could not delete that memo.');
    }
  }));

  document.querySelectorAll('[data-memo-title]').forEach(input => input.addEventListener('change', async e => {
    const id = e.target.getAttribute('data-memo-title');
    const value = e.target.value.trim();
    if (!value) return;
    try { await voiceMemoStore.update(id, { title: value }); await refreshVoiceMemosInMemory(); }
    catch (_) { showToast('Could not update the memo title.'); }
  }));

  document.querySelectorAll('[data-memo-course]').forEach(select => select.addEventListener('change', async e => {
    const id = e.target.getAttribute('data-memo-course');
    try { await voiceMemoStore.update(id, { courseId: e.target.value || null }); await refreshVoiceMemosInMemory(); }
    catch (_) { showToast('Could not update the course tag.'); }
  }));

  for (const memo of state.voiceMemos) {
    const priorAudio = voicePlayerElements.get(memo.id);
    if (priorAudio) {
      priorAudio.pause();
      priorAudio.remove();
      revokeObjectUrl(voicePlayerUrls.get(memo.id));
      voicePlayerElements.delete(memo.id);
      voicePlayerUrls.delete(memo.id);
    }
    ...
    voicePlayerElements.set(memo.id, audio);

    try {
      const loaded = await voiceMemoStore.get(memo.id);
      if (loaded?.blob) {
        const url = URL.createObjectURL(loaded.blob);
        voicePlayerUrls.set(memo.id, url);
        audio.src = url;
      }
    } catch (e) { console.warn('Unable to load memo audio:', memo.id, e); }

    audio.addEventListener('loadedmetadata', () => {
      const d = document.querySelector(`[data-memo-duration="${memo.id}"]`);
      if (d && Number.isFinite(audio.duration)) d.textContent = formatMemoTime(audio.duration);
      renderMemoWaveLoop(memo.id);
    });
    audio.addEventListener('timeupdate', () => {
      const c = document.querySelector(`[data-memo-current="${memo.id}"]`);
      if (c) c.textContent = formatMemoTime(audio.currentTime);
      renderMemoWaveLoop(memo.id);
    });
    audio.addEventListener('ended', () => {
      renderMemoWaveLoop(memo.id);
      const btn = document.querySelector(`[data-memo-play="${memo.id}"]`);
      if (btn) btn.innerHTML = icon('play');
    });
  }

  document.querySelectorAll('[data-memo-play]').forEach(btn => btn.addEventListener('click', () => {
    const id = btn.getAttribute('data-memo-play');
    const audio = voicePlayerElements.get(id);
    if (!audio?.src) { showToast('Memo audio is still loading.'); return; }
    if (audio.paused) {
      audio.play().then(() => { btn.innerHTML = icon('pause'); renderMemoWaveLoop(id); }).catch(() => showToast('Unable to play this memo.'));
    } else {
      audio.pause(); btn.innerHTML = icon('play'); renderMemoWaveLoop(id);
    }
  }));

  document.querySelectorAll('[data-memo-rewind]').forEach(btn => btn.addEventListener('click', () => {
    const audio = voicePlayerElements.get(btn.getAttribute('data-memo-rewind'));
    if (audio) audio.currentTime = Math.max(0, audio.currentTime - 10);
  }));

  document.querySelectorAll('[data-memo-forward]').forEach(btn => btn.addEventListener('click', () => {
    const audio = voicePlayerElements.get(btn.getAttribute('data-memo-forward'));
    if (audio) audio.currentTime = Math.min(audio.duration || Infinity, audio.currentTime + 10);
  }));

  document.querySelectorAll('[data-memo-speed]').forEach(btn => btn.addEventListener('click', () => {
    const id = btn.getAttribute('data-memo-speed');
    const audio = voicePlayerElements.get(id);
    if (!audio) return;
    audio.playbackRate = Number(btn.getAttribute('data-speed')) || 1;
    btn.parentElement.querySelectorAll('.voice-speed-btn').forEach(x => x.classList.remove('active'));
    btn.classList.add('active');
  }));

  document.querySelectorAll('[data-memo-wave]').forEach(canvas => canvas.addEventListener('click', e => {
    const id = canvas.getAttribute('data-memo-wave');
    const audio = voicePlayerElements.get(id);
    if (!audio?.duration) return;
    const rect = canvas.getBoundingClientRect();
    const ratio = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
    audio.currentTime = ratio * audio.duration;
    renderMemoWaveLoop(id);
  }));

  // Clean up stale player nodes caused by re-renders.
  const activeIds = new Set(state.voiceMemos.map(m => m.id));
  for (const [id, audio] of voicePlayerElements) {
    if (!activeIds.has(id)) {
      audio.pause();
      audio.remove();
      revokeObjectUrl(voicePlayerUrls.get(id));
...
function bootstrap() {
  try {
    ...
    // 1b. Initialize standalone voice memo storage and migrate legacy localStorage memos.
    initializeVoiceMemos().catch(() => {});
