...
const ICONS = {
  ...
  aiSearch: `<svg viewBox="0 0 24 24"><circle cx="10.8" cy="10.8" r="6.8"/><line x1="16" y1="16" x2="21" y2="21"/><path d="M11 6.7l.9 2.7 2.7.9-2.7.9-.9 2.7-.9-2.7-2.7-.9 2.7-.9z"/></svg>`,
...
/* =========================================================================
   ...
   ========================================================================= */
let state = {
  view: 'today', // today | calendar | courses | settings
  ...
  try {
    ...
    try {
      if (state.view === 'today') {
        ...
        viewHtml = renderUnifiedAiSearchView();
...
function renderBottomNav() {
  const tabs = [
    ...
    { id: 'ai', icon: 'aiSearch', label: 'AI and Search' },
    { id: 'settings', icon: 'settings', label: 'Settings' }
  ];
  return `<nav class="bottom-nav-wrap" aria-label="Primary navigation"><div class="bottom-nav">${tabs.map(t => `<button class="nav-item ${state.view===t.id?'active':''}" data-nav="${t.id}" type="button" aria-label="${t.label}">${icon(t.icon)}</button>`).join('')}</div></nav>`;
}
            ...
            <div class="surface-content" style="padding:16px;">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;">
                <div style="min-width:0;">
                  <h3 style="margin:0 0 4px;font-size:1rem;">${escapeHtml(n.title)}</h3>
                  <span style="font-size:0.72rem;color:var(--muted);">${new Date(n.updatedAt).toLocaleDateString()}</span>
                </div>
                <button class="icon-btn sm note-delete-btn" data-delete-note="${escapeHtml(n.id)}" type="button" aria-label="Delete note" title="Delete note">${icon('trash')}</button>
              </div>
              <div style="font-size:0.88rem;line-height:1.6;margin:8px 0;white-space:pre-wrap;">${escapeHtml(n.content)}</div>
            </div>
          `).join('')}
        </div>
      ` : `
...
function renderSettingsView() {
  ...
  const syncState = failedJobs > 0 ? 'Needs attention' : activeJobs > 0 ? `${activeJobs} processing` : isSupabaseConfigured() ? 'Synced' : 'Local only';
  const syncDot = failedJobs > 0 ? 'attention' : activeJobs > 0 ? 'busy' : 'ready';
  return `
    <div class="settings-page">
      <div class="settings-title-row"><div><div class="eyebrow">School Center</div><h2 class="headfont">Settings</h2><p>Search, appearance, cloud sync, and AI connection.</p></div></div>
      <div class="panel settings-card">
        <div class="section-head-row"><div><h3 class="headfont">Cloud Sync</h3><div class="section-sub">Background jobs and remote data synchronization.</div></div><span class="settings-sync-status"><span class="ai-connection-dot ${syncDot}"></span>${syncState}</span></div>
        ...
        <details class="settings-advanced"><summary>Cloud connection</summary><div class="field-label">Supabase Project URL</div><input type="text" id="supabase-url-field" class="search-input" value="${currentSupabaseUrl}"><div class="field-label">Supabase Anon Key</div><input type="password" id="supabase-key-field" class="search-input" value="${currentSupabaseKey}"><button class="btn-ghost" id="save-cloud-settings-btn" type="button">Save cloud config</button></details>
      </div>
      <div class="panel settings-card">
        <h3 class="headfont">Appearance</h3><div class="field-label">Atmosphere preset</div>
        <div class="theme-row" id="theme-row">${Object.entries(THEME_PRESETS).map(([k,t])=>`<button class="swatch ${currentTheme.label===t.label?'active':''}" data-theme-key="${k}" type="button" style="background:linear-gradient(135deg, ${t.accent}, ${t.lava[1]})" aria-label="${t.label}"></button>`).join('')}</div>
        <div class="field-label">Lava lamp opacity</div><input type="range" min="0" max="1" step="0.05" value="1" id="lava-slider" style="width:100%;accent-color:var(--accent);">
      </div>
      <div class="panel settings-card">
        <h3 class="headfont">AI Connection</h3><p class="section-sub">The AI workspace uses the key saved in this browser.</p>
        <div class="field-label">Gemini API key</div><input type="password" id="more-gemini-key-field" class="search-input" value="${getGeminiApiKey()}" autocomplete="off" placeholder="Paste Gemini API key">
...
function renderUnifiedAiSearchView() {
  const q = String(state.unifiedQuery || '').trim();
  const results = q ? performUniversalSearch(q, COURSES) : [];
  const attachments = state.aiAttachments || [];
  const hasMessages = (state.aiChatMessages || []).some(m => !m.isThinking);
  const showSearchResults = q && results.length > 0;

  return `
    <section class="ai-workspace ai-workspace-page" role="region" aria-label="AI and search workspace">
      <header class="ai-workspace-head">
        <div class="ai-workspace-title"><span class="ai-workspace-spark">${icon('spark')}</span><span class="ai-launch-dot ${state.aiConnectionState}" aria-hidden="true"></span></div>
      </header>

      ${showSearchResults ? `
        <div class="unified-search-results" aria-live="polite">
          <div class="unified-search-count">${results.length} result${results.length === 1 ? '' : 's'}</div>
          ${results.map(r => `
            <button class="unified-search-result" data-unified-nav="${escapeHtml(r.type)}" data-course-id="${escapeHtml(r.courseId || '')}" data-target-tab="${escapeHtml(r.targetTab || 'overview')}" data-assignment-id="${escapeHtml(r.assignmentId || '')}" type="button">
              <span class="unified-search-result-icon">${icon(r.type === 'assignment' ? 'clipboard' : r.type === 'course' ? 'book' : r.type === 'note' ? 'doc' : 'search')}</span>
              <span class="unified-search-result-copy"><b>${escapeHtml(r.title)}</b><small>${escapeHtml(r.subtitle || r.snippet || '')}</small></span>
              <span class="unified-search-result-badge">${escapeHtml(r.badge || '')}</span>
            </button>`).join('')}
        </div>
      ` : `
        <div class="ai-chat-stream" id="ai-chat-stream" role="log" aria-live="polite">
          ${!hasMessages && !state.aiIsThinking ? `<div class="ai-empty-state"><div class="ai-empty-symbol">${icon('spark')}</div><h2>How can I help?</h2><p>Search School Center or ask the integrated AI workspace for help.</p></div>` : ''}
      ...
      <div class="ai-composer-shell unified-ai-composer-shell">
        ...
        <div class="ai-composer">
          ...
          <textarea id="ai-chat-input" class="ai-composer-input" rows="1" placeholder="Search or message School Center…" ${state.aiIsThinking ? 'disabled' : ''}>${escapeHtml(state.unifiedQuery || '')}</textarea>
          <button class="ai-composer-icon ai-mic-button ${state.aiTranscribing ? 'active' : ''}" id="ai-mic-btn" type="button" aria-label="Transcribe speech" title="Transcribe speech" ${state.aiIsThinking ? 'disabled' : ''}>${icon('mic')}</button>
          <button class="ai-send-button" id="ai-chat-send" type="button" aria-label="Send message" title="Send" ${state.aiIsThinking || (q && results.length && !attachments.length) ? 'disabled' : ''}>${icon('arrowUp')}</button>
        </div>
        <input id="ai-file-input" type="file" hidden multiple accept="*/*" />
        <div class="ai-composer-status"><span id="ai-live-transcript">${state.aiTranscribing ? 'Listening…' : ''}</span></div>
      </div>
    </section>
  `;
}
...
function attachEventHandlers() {
  document.querySelectorAll('[data-nav]').forEach(el => {
    el.addEventListener('click', () => {
      const nextView = el.getAttribute('data-nav');
      state.view = nextView;
      state.courseId = null;
      if (nextView !== 'ai') state.unifiedQuery = '';
      render();
      if (nextView === 'ai') setTimeout(() => document.getElementById('ai-chat-input')?.focus(), 50);
    });
  });
  ...
  if (syncPull) syncPull.addEventListener('click', async () => {
    ...
    try { await syncDataFromSupabase(); showToast('Cloud sync complete ✓'); }
    catch (_) { showToast('Sync unavailable — continuing locally'); }
    render();
  });
  const syncPush = document.getElementById('sync-push-btn');
  if (syncPush) syncPush.addEventListener('click', () => showToast('Local state is already saved in this browser.'));
  ...
  // No header-level AI/search controls: the unified workspace lives only in bottom navigation.
  const headerAiBtn = document.getElementById('header-ai-btn');
  if (headerAiBtn) headerAiBtn.remove();
  const headerSearchBtn = document.getElementById('quick-search-btn');
  if (headerSearchBtn) headerSearchBtn.remove();
  ...
  if (aiInput && aiSend) {
    const submit = async () => {
      ...
      const searchMatches = text ? performUniversalSearch(text, COURSES) : [];
      if (text && searchMatches.length && !files.length) return;
      if (!text && !files.length) return;
      aiActiveAttachments = files;
      state.aiAttachments = [];
      state.unifiedQuery = '';
      state.aiChatMessages.push({ sender: 'user', text: text || 'Please review these attached files.', attachments: files.map(f => f.name) });
      state.aiChatMessages.push({ sender: 'assistant', text: '', isThinking: true });
      state.aiIsThinking = true;
      state.aiConnectionState = 'busy';
      aiInput.value = '';
      render();
      setTimeout(() => { const stream = document.getElementById('ai-chat-stream'); if (stream) stream.scrollTop = stream.scrollHeight; }, 20);
      try {
        ...
        state.aiChatMessages[last] = { sender: 'assistant', text: formatted, isThinking: false };
        state.aiConnectionState = 'ready';
      } catch (err) {
        console.error(err);
        const last = state.aiChatMessages.length - 1;
        const message = escapeHtml(err.message || 'Unable to connect to the AI service.');
        state.aiChatMessages[last] = { sender: 'assistant', text: `<div class="ai-inline-error">${message}</div>`, isThinking: false };
        state.aiConnectionState = 'attention';
      } finally {
        aiActiveAttachments = [];
        state.aiIsThinking = false;
        render();
        setTimeout(() => { const stream = document.getElementById('ai-chat-stream'); if (stream) stream.scrollTop = stream.scrollHeight; }, 40);
      }
    };
    ...
    aiInput.addEventListener('input', () => {
      state.unifiedQuery = aiInput.value;
      const currentHeight = Math.min(aiInput.scrollHeight, 160);
      render();
      setTimeout(() => {
  ...
  document.querySelectorAll('[data-unified-nav]').forEach(item => item.addEventListener('click', () => {
    const type = item.getAttribute('data-unified-nav');
    const courseId = item.getAttribute('data-course-id') || '';
    const targetTab = item.getAttribute('data-target-tab') || 'overview';
    const assignmentId = item.getAttribute('data-assignment-id') || '';
    state.unifiedQuery = '';
    if (type === 'assignment') {
      state.view = 'courses';
      state.courseId = courseId || null;
      state.courseTab = 'assignments';
      state.assignmentDetailId = assignmentId || null;
    } else if (type === 'note' || type === 'material' || type === 'lecture' || type === 'concept') {
      state.view = 'courses';
      state.courseId = courseId || null;
      state.courseTab = targetTab === 'notes' ? 'notes' : targetTab === 'materials' ? 'materials' : 'overview';
    } else if (type === 'course') {
      state.view = 'courses';
      state.courseId = courseId || null;
  ...
  if (!window._sc_keydown_attached) {
    ...
    window.addEventListener('keydown', e => {
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') { e.preventDefault(); state.view = 'ai'; state.courseId = null; state.unifiedQuery = ''; render(); setTimeout(() => document.getElementById('ai-chat-input')?.focus(), 50); return; }
      if (e.key === 'Escape') {
        if (state.aiTranscribing) { stopAiLiveTranscription(); }
...
async function toggleAiLiveTranscription() {
  ...
  const apiKey = getGeminiApiKey();
  if (!apiKey) { showToast('Connect Gemini in Settings before using the microphone.'); return; }
  state.aiConnectionState = 'busy';
  aiLiveTranscriber = new GeminiLiveTranscriber({
    ...
    onFinal: text => {
      ...
      if (input && text) {
        ...
        state.unifiedQuery = input.value;
        input.focus();
      }
      state.aiLiveTranscript = '';
      const live = document.getElementById('ai-live-transcript');
