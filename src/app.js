...
import { loadAiChatHistory, persistAiMessage, getCurrentAiUser, sendAiMagicLink, signOutAiCloud, getEphemeralLiveToken, subscribeToAiChatHistory } from './ai-history.js';
...
function renderBottomNav() {
  ...
  return `<nav class="bottom-nav-wrap" aria-label="Primary navigation"><div class="bottom-nav">${tabs.map(t => `<button class="nav-item ${state.view===t.id?'active':''}" data-nav="${t.id}" type="button" aria-label="${t.label}" title="${t.label}">${icon(t.icon)}</button>`).join('')}</div></nav>`;
}
...
function renderAiConversation() {
  ...
  return messages.map(m => {
    ...
    return `<article class="ai-message ${sender}">${renderAiMessageAttachments(m.attachments || [])}<div class="ai-message-body">${text}</div><time>${new Date(m.createdAt || Date.now()).toLocaleTimeString([], {hour:'numeric', minute:'2-digit'})}</time></article>`;
  }).join('');
}
...
function renderSettingsView() {
  ...
  const syncState = failedJobs > 0 ? 'Needs attention' : activeJobs > 0 ? `${activeJobs} processing` : isSupabaseConfigured() ? 'Connected' : 'Local only';
  const syncDot = failedJobs > 0 ? 'attention' : activeJobs > 0 ? 'busy' : isSupabaseConfigured() ? 'ready' : 'idle';
  ...
  return `
    <div class="settings-page">
      ...
      <div class="panel settings-card">
        <div class="section-head-row"><div><h3 class="headfont">Cloud Sync</h3><div class="section-sub">Assignments, notes, materials, and AI history can stay synchronized across devices.</div></div><span class="settings-sync-status"><span class="ai-connection-dot ${syncDot}"></span>${syncState}</span></div>
      ...
      <div class="panel settings-card">
        <h3 class="headfont">AI history account</h3>
        <div class="section-sub">Sign in once with the same email on your other devices so your AI conversations follow you.</div>
        ${authEmail ? `
          <div class="settings-account-row"><div><b>${escapeHtml(authEmail)}</b><div class="settings-account-state"><span class="ai-connection-dot ready"></span> Cloud history connected</div></div><button class="btn-ghost" id="ai-signout-btn" type="button">Sign out</button></div>
        ` : `
          ...
          <div class="settings-account-help">A magic link opens this site and keeps the same AI history on your other devices.</div>
        `}
      </div>
...
function attachEventHandlers() {
  ...
  if (aiSignout) aiSignout.addEventListener('click', async () => { try { await signOutAiCloud(); if (aiHistoryUnsubscribe) { aiHistoryUnsubscribe(); aiHistoryUnsubscribe = null; } state.aiCloudUser=null; state.aiCloudConnected=false; render(); showToast('Signed out.'); } catch (e) { showToast(e.message); } });
