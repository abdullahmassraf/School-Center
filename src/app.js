...
import { pushLocalDataToCloud, pullCloudDataToLocal, fullTwoWaySync } from './data-sync.js';
...
function bootstrap() {
  try {
    ...
    // 4b. Auto-push notes/assignments to the cloud shortly after any local
    // change, when signed in, so "Push to Cloud" isn't the only way changes
    // leave this device. Debounced so rapid edits (typing, etc.) collapse
    // into one request instead of one per keystroke.
    let autoPushTimer = null;
    const scheduleAutoPush = () => {
      if (autoPushTimer) clearTimeout(autoPushTimer);
      autoPushTimer = setTimeout(() => { pushLocalDataToCloud().catch(() => {}); }, 2500);
    };
    try { notesManager.subscribe(scheduleAutoPush); } catch (e) {}
    try { assignmentsManager.subscribe(scheduleAutoPush); } catch (e) {}
    ...
    try {
      ...
      sb?.auth?.onAuthStateChange(async (_event, session) => {
        ...
        if (state.aiCloudUser?.id) {
          // A sign-in (including on a new device) also pulls this account's
          // notes and assignments down, so they follow the same magic-link
          // identity as AI chat history instead of staying device-local.
          pullCloudDataToLocal().then(() => { if (state.view === 'courses' || state.view === 'today') render(); }).catch(() => {});
        }
