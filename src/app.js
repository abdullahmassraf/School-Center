...
function renderSavedMemosSection() {
  ...
  if (!memos.length) return `<div style="font-size:0.78rem;color:var(--muted);text-align:center;padding:10px 0;">No saved memos yet — record one above!</div>`;
  return memos.map((m, i) => `
    <div class="saved-memo-row" style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:rgba(255,255,255,0.04);border-radius:12px;border:1px solid var(--hairline);margin-bottom:8px;">
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:0.72rem;color:var(--muted);">${m.duration} · ${new Date(m.savedAt).toLocaleString([], {dateStyle:'short',timeStyle:'short'})}</div>
      </div>
...
function attachEventHandlers() {
  ...
  if (stopBtn) {
    stopBtn.addEventListener('click', async () => {
      ...
      if (statusLabel) statusLabel.textContent = 'Recording captured — preview below.';
      ...
      if (area && audioResult) {
        ...
        area.innerHTML = `
          <div class="audio-player-card" style="padding:14px;background:rgba(255,255,255,0.06);border-radius:14px;border:1px solid var(--hairline);">
            <div style="font-size:0.82rem;font-weight:700;margin-bottom:8px;">New Recording — ${durStr}</div>
